...
public class ChunkFinder extends Module {
    ...
    enum ChunkColor { GREEN, RED, WHITE, YELLOW, CYAN, ORANGE }
    ...
    public void onTick() {
        if (mc.world == null) {
            ...
            this.notifiedChunks.clear(); this.chunkData.clear();
            this.scanQueue.clear(); this.lastPlayerChunk = null;
            return;
        }
    ...
    private void tryStartScans() {
        ...
        while (this.activeScans.get() < MAX_CONCURRENT_SCANS && !this.scanQueue.isEmpty()) {
            ...
            this.pool.submit(() -> {
                try { this.analyzeChunk(pos); }
                finally { this.activeScans.decrementAndGet(); }
            });
        }
    }
        ...
        for (int x = 0; x < 16; ++x) {
            for (int z = 0; z < 16; ++z) {
                for (int y = worldMinY; y <= worldMaxY; ++y) {
                    ...
                    if (!analysis.hasDioriteVein && isTargetBlock(state)) {
                        long posLong = mutable.asLong();
                        if (!dioriteVisited.contains(posLong)) {
                            BlockPos immutable = mutable.toImmutable();
                            int lenUp   = countVerticalRun(immutable, Direction.UP);
                            int lenDown = countVerticalRun(immutable, Direction.DOWN);
                    ...
                    if (!analysis.hasObsidianVein && y >= 15 && y <= 63 && state.isOf(Blocks.OBSIDIAN)) {
                        ...
                        if (!obsidianVisited.contains(posLong)) {
                            BlockPos immutable = mutable.toImmutable();
                            int lenUp   = countVerticalRunObsidian(immutable, Direction.UP);
                            int lenDown = countVerticalRunObsidian(immutable, Direction.DOWN);
                            int total   = lenUp + 1 + lenDown;
                            if (total >= 15) {
                                BlockPos veinStart = immutable.offset(Direction.DOWN, lenDown);
                                boolean enclosed = true;
                            ...
                            if (isVineTop) {
                                ...
                                while (current.getY() >= Math.max(worldMinY, 40)) {
                                    if (mc.world.getBlockState(current).getBlock() == Blocks.VINE) {
                                        ++vineLength; current = current.down();
                                    } else break;
                                }
                                if (vineLength >= 37) {
                                ...
                                if (isKelpBase) {
                                    ...
                                    while (current.getY() <= worldMaxY) {
                                        ...
                                        if (cs.getBlock() == Blocks.KELP || cs.getBlock() == Blocks.KELP_PLANT) {
                                            ++kelpLength; current = current.up(); continue;
                                        }
                                        if (cs.getFluidState().isEmpty()) reachedSurface = true;
                                        break;
                                    }
                                    if (!(kelpLength < 6 && reachedSurface)) {
                                        ++kelpPlantsFound;
                                        if (reachedSurface) ++fullKelpPlants;
                                    }
                                }
                            }
                        }
    ...
    private int countVerticalRun(BlockPos from, Direction dir) {
        ...
        while (count <= 20) {
            m.move(dir);
            if (!isTargetBlock(mc.world.getBlockState(m))) break;
            ++count;
        }
        return count;
    }
    ...
    private void evaluateChunk(ChunkPos pos, ChunkAnalysis analysis) {
        if (ignorePlayerChunk && pos.equals(this.lastPlayerChunk)) return;
        if (detectItems && this.chunkItemCounts.getOrDefault(pos, 0) > maxItems) return;
        if (detectXP.getValue() && this.chunkXPCounts.getOrDefault(pos, 0) > maxXP.getValue()) return;

        boolean suspicious = false;
        List<String> reasonList = new ArrayList<>();

        if (analysis.rotatedCount >= rotatedThreshold.getValue()) {
            suspicious = true;
            reasonList.add("Rotated:" + analysis.rotatedCount);
        }
        if (analysis.hasLongDripstone) { suspicious = true; reasonList.add("Dripstone"); }
        if (analysis.hasLongVine)      { suspicious = true; reasonList.add("Vine"); }
        if (analysis.allKelpFull)      { suspicious = true; reasonList.add("Kelp"); }
        if (analysis.hasDioriteVein)   { suspicious = true; reasonList.add("DioriteVein"); }
        if (analysis.hasObsidianVein)  { suspicious = true; reasonList.add("ObsidianVein"); }

        if (!suspicious) return;

        if (this.flaggedChunks.add(pos) && this.notifiedChunks.add(pos)) {
            StringBuilder reasons = new StringBuilder();
            for (String r : reasonList) { reasons.append(r).append(" "); }
